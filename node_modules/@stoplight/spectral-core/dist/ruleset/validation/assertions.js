...
function assertValidRuleset(ruleset, format = 'js', sourceContext) {
    var _a, _b;
    if (!(0, json_1.isPlainObject)(ruleset)) {
        throw new errors_1.RulesetValidationError('invalid-ruleset-definition', 'Provided ruleset is not an object', [], sourceContext === undefined ? undefined : { source: sourceContext.source });
    }
    if (!('rules' in ruleset) && !('extends' in ruleset) && !('overrides' in ruleset)) {
        throw new errors_1.RulesetValidationError('invalid-ruleset-definition', 'Ruleset must have rules or extends or overrides defined', [], sourceContext === undefined
            ? undefined
            : { source: sourceContext.source, range: (_a = sourceContext.getLocationForJsonPath([])) === null || _a === void 0 ? void 0 : _a.range });
    }
    const validate = (0, ajv_1.createValidator)(format);
    if (!validate(ruleset)) {
        throw new es_aggregate_error_1.default((0, errors_1.convertAjvErrors)((_b = validate.errors) !== null && _b !== void 0 ? _b : [], sourceContext));
    }
}
