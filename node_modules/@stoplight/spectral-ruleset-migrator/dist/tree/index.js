...
const path = tslib_1.__importStar(require("@stoplight/path"));
const astring = tslib_1.__importStar(require("astring"));
const commonjs_1 = require("./commonjs");
const esm_1 = require("./esm");
const requireResolve_1 = tslib_1.__importDefault(require("../requireResolve"));
...
class Tree {
    constructor({ format, npmRegistry, scope }) {
        ...
        this.scope = scope;
        tslib_1.__classPrivateFieldSet(this, _Tree_npmRegistry, npmRegistry !== null && npmRegistry !== void 0 ? npmRegistry : null, "f");
        tslib_1.__classPrivateFieldSet(this, _Tree_module, format === 'commonjs' ? commonjs_1.commonjs : esm_1.esm, "f");
        if (format === 'commonjs' && tslib_1.__classPrivateFieldGet(this, _Tree_npmRegistry, "f") !== null) {
            throw new Error(`'npmRegistry' option must not be used with commonjs output format.`);
        }
    }
    addImport(specifier, source, _default = false) {
        const existingImportDeclaration = tslib_1.__classPrivateFieldGet(this, _Tree_importDeclarations, "f").get(source);
        ...
        if (existingImportDeclaration === void 0) {
            ...
            tslib_1.__classPrivateFieldGet(this, _Tree_importDeclarations, "f").set(source, [
    ...
    toString() {
        ...
        tslib_1.__classPrivateFieldGet(this, _Tree_module, "f").dependencies.clear();
        return astring.generate(ast_types_1.builders.program([
            ...Array.from(tslib_1.__classPrivateFieldGet(this, _Tree_importDeclarations, "f").entries())
                .sort(sortImports)
                .flatMap(([source, identifiers]) => {
                const resolvedSource = tslib_1.__classPrivateFieldGet(this, _Tree_npmRegistry, "f") !== null && !tslib_1.__classPrivateFieldGet(this, _Tree_resolvedPaths, "f").has(source) && !source.startsWith(tslib_1.__classPrivateFieldGet(this, _Tree_npmRegistry, "f"))
                    ? path.join(tslib_1.__classPrivateFieldGet(this, _Tree_npmRegistry, "f"), source)
                    : source;
                const nonDefault = identifiers.filter(({ default: _default }) => !_default).sort(sortMembers);
                return [
                    ...(nonDefault.length > 0
                        ? [
                            tslib_1.__classPrivateFieldGet(this, _Tree_module, "f").importDeclaration(nonDefault.map(({ imported, local }) => [imported, local]), resolvedSource),
                        ]
                        : []),
                    ...identifiers
                        .filter(({ default: _default }) => _default)
                        .flatMap(({ local }) => tslib_1.__classPrivateFieldGet(this, _Tree_module, "f").importDefaultDeclaration(local, resolvedSource)),
                ];
            }),
            tslib_1.__classPrivateFieldGet(this, _Tree_module, "f").exportDefaultDeclaration(this.ruleset),
            ...tslib_1.__classPrivateFieldGet(this, _Tree_module, "f").dependencies,
        ]));
    }
    ...
    resolveModule(identifier, ctx, kind) {
        ...
        if (path.isURL(identifier) || path.isAbsolute(identifier)) {
            resolved = identifier;
            tslib_1.__classPrivateFieldGet(this, _Tree_resolvedPaths, "f").add(identifier);
        }
            ...
            resolved = path.join(ctx.filepath, '..', identifier);
            tslib_1.__classPrivateFieldGet(this, _Tree_resolvedPaths, "f").add(resolved);
        }
        return resolved;
    }
    fork() {
